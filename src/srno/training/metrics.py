...
def accumulate_trajectory_metrics(
    ...
    *,
    ...
    aperture = (prediction.aperture - target.aperture).abs()
    ...
    accumulator.add("predicted_terminal_aperture_m", prediction.aperture[:, -1])
    accumulator.add("target_terminal_aperture_m", target.aperture[:, -1])
    ...
    predicted_lag = prediction.aperture - schedule
    target_lag = target.aperture - schedule
